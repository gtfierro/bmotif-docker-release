...
from buildingmotif.namespaces import RDF, SH, BRICK, OWL
...
def defn_to_shape(shapename, varname, defn, shape_graph, ns, path = None):
    # Use fresh blank nodes for nested shapes to encourage inlining in the TTL output
    # If shapename is None we are constructing a nested shape; force a BNode
    if shapename is None:
        varname = BNode()
    else:
        # Top-level shapes can reuse provided identifier or fall back to a BNode
        varname = varname or BNode()
...
def prop_to_shape(shapename: URIRef, shape_graph: Graph, varname: URIRef, propname: URIRef, value: Any, ns):
    ...
    if isinstance(value, str):
        # treat like a property shape; ensure we use the correct path for this property
        print(f"string value: {value}")
        return string_to_shape(shapename, shape_graph, varname, value, path=propname)
    ...
    def _consume_edges(vdict, edge_stack):
        key, inner = list(vdict.items())[0]
        edge_uri = BRICK[key]
        # avoid duplicating the same edge
        if not edge_stack or edge_stack[-1] != edge_uri:
            edge_stack.append(edge_uri)
        print(f"adding {key} to edge stack: {edge_stack}")
        if isinstance(inner, dict):
            print(f"recursing on {inner}")
            if inner:
                nkey, nval = list(inner.items())[0]
                # if the next level is another edge, keep consuming
                if any([nkey.startswith(edge) for edge in possible_edges]):
                    return _consume_edges(inner, edge_stack)
                # otherwise, we've hit a class at the end of this path; build shape with composed path
                property_path = edge_list_to_property_path(edge_stack, shape_graph)
                return prop_to_shape(shapename, shape_graph, varname, property_path, {nkey: nval}, ns)
            # empty dict fallback (unlikely)
            property_path = edge_list_to_property_path(edge_stack, shape_graph)
            return string_to_shape(shapename, shape_graph, varname, "", path=property_path)
        # inner is a string terminal; use the composed property path
        print(f"returning {inner} as leftover (string)")
        property_path = edge_list_to_property_path(edge_stack, shape_graph)
        return string_to_shape(shapename, shape_graph, varname, inner, path=property_path)
    ...
    for key, vdict in value.items():
        ...
        if any([key.startswith(edge) for edge in possible_edges]):
            ...
            # the 'leftover' is a property shape on qvs (do not attach to top-level shapename)
            leftover = defn_to_shape(None, varname, vdict, shape_graph, ns)
