...
from buildingmotif.label_parsing.tokens import Token, Constant, Identifier
from buildingmotif.label_parsing.combinators import abbreviations, string, choice, regex

ParserDict = TypedDict("ParserDict", {"parser": str, "args": dict})
logger = logging.getLogger(__name__)

class EquipAbbreviations(abbreviations):
...
class PointAbbreviations(abbreviations):
    def __init__(self, id=None):
        patterns = {
            'STAT': BRICK.Status,
...
class EquipID(regex):
    def __init__(self, id=None):
        super().__init__(r"\d+[A-Z]", Identifier, id=id)


MY_PARSERS = {
    "EquipAbbreviations": EquipAbbreviations,
    "PointAbbreviations": PointAbbreviations,
    "EquipID": EquipID,
}
...
def _get_parser_by_name(parser_name: str) -> Type[Parser]:
    ...
    :return: parser class
    :rtype: Type[Parser]"""
    logger.info(f"Getting parser by name: {parser_name} {type(parser_name)}")
    parsers = Parser.__subclasses__()
    for parser in parsers:
        if parser.__name__ == parser_name:
            return parser
    if parser_name in MY_PARSERS:
        return MY_PARSERS[parser_name]
    raise NameError(f'ppppParser of type "{parser_name}" does not exist')
...
def deserialize(parser_dict: Union[ParserDict, dict]) -> Parser:
    ...
    logger.info(f"Deserializing parser {parser_dict['parser']} with args {args_dict}")
